...
FiberLyse needs Tkinter to open the desktop window.
...
On Windows or macOS, installing Python from python.org usually includes Tkinter.
...
SystemTimestamp
LedState
G0 or another G column
```

---

### No isosbestic or excitatory samples found

Check that your `LedState` column contains:

```text
1 for isosbestic/reference signal
2 for excitatory/signal channel
```
...
FiberLyse is meant for visual inspection and interactive analysis.

Before using exported results, it is recommended to check:
